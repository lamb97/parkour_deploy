...
namespace legged
{
  bool UnitreeHW::init(ros::NodeHandle &root_nh, ros::NodeHandle &robot_hw_nh)
  {
    ...
    robot_hw_nh.getParam("loop_frequency", loop_frequency_);
    ...
    loop_udp_send_ptr_ = std::make_shared<UNITREE_LEGGED_SDK::LoopFunc>("udp_send", 1/loop_frequency_, 4, boost::bind(&UnitreeHW::UDPSend, this));
    loop_udp_recv_ptr_ = std::make_shared<UNITREE_LEGGED_SDK::LoopFunc>("udp_recv", 1/loop_frequency_, 4, boost::bind(&UnitreeHW::UDPRecv, this));

    loop_udp_send_ptr_->start();
    loop_udp_recv_ptr_->start();
  ...
  void UnitreeHW::write(const ros::Time & /*time*/, const ros::Duration & /*period*/)
  {
    ...
    // for (int i = 0; i < 12; ++i) {
    //     ROS_WARN("Joint %d: PosDes = %f, VelDes = %f, Kp = %f, Kd = %f, Ff = %f",
    //              i, jointData_[i].posDes_, jointData_[i].velDes_, jointData_[i].kp_, jointData_[i].kd_, jointData_[i].ff_);
    // }

    // for (int i = 0; i < 12; ++i)
    // {
    //   lowCmd_.motorCmd[i].q = 0;
    //   lowCmd_.motorCmd[i].dq = 0;
    //   lowCmd_.motorCmd[i].Kp = 0;
  ...
  void UnitreeHW::updateJoystick(const ros::Time &time)
  {
    ...
    joyMsg.buttons.push_back(keyData.btn.components.F1);
  ...
  void UnitreeHW::UDPRecv()
  {
    udp_->Recv();
  }

  void UnitreeHW::UDPSend()
  {
    udp_->Send();
  }
